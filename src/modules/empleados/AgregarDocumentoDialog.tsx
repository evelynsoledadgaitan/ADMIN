...
import { validarDocumento, hayErrores } from './validaciones'
import { valoresDocumentoVacio, ETIQUETAS_TIPO_DOCUMENTO, type DocumentoFormValues, type TipoDocumento } from './types'

const OPCIONES_TIPO_DOCUMENTO = Object.entries(ETIQUETAS_TIPO_DOCUMENTO).map(([value, label]) => ({ value, label }))

export interface AgregarDocumentoDialogProps {
  empleadoId: string
  abierto: boolean
...
/**
 * Agregar documento — a diferencia del resto de los adjuntos del sistema
 * (siempre opcionales), acá el archivo es obligatorio: un documento sin
 * archivo no tiene ningún sentido, es la razón de ser de esta pantalla.
 */
export function AgregarDocumentoDialog({ empleadoId, abierto, onOpenChange }: AgregarDocumentoDialogProps) {
  const toast = useToast()
  const agregar = useAgregarDocumento(empleadoId)

  const [valores, setValores] = React.useState<DocumentoFormValues>(valoresDocumentoVacio())
  const [archivo, setArchivo] = React.useState<File | null>(null)
  const [errores, setErrores] = React.useState<ReturnType<typeof validarDocumento>>({})
  const [errorArchivo, setErrorArchivo] = React.useState<string | undefined>()

  React.useEffect(() => {
    if (abierto) {
      setValores(valoresDocumentoVacio())
      setArchivo(null)
      setErrores({})
  ...
  function actualizar<K extends keyof DocumentoFormValues>(campo: K, valor: DocumentoFormValues[K]) {
    setValores((actuales) => ({ ...actuales, [campo]: valor }))
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const erroresValidacion = validarDocumento(valores)
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 z-50 max-h-[85vh] overflow-y-auto rounded-t-2xl border-t border-border bg-surface p-5 shadow-xl animar-entrada-pantalla sm:inset-x-auto sm:left-1/2 sm:top-1/2 sm:bottom-auto sm:w-full sm:max-w-sm sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl sm:border">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Select
              ...
              onValueChange={(v) => actualizar('tipo_documento', v as TipoDocumento)}
